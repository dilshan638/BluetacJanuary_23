...
export function signupUser(newUserData,userType) {
    return axios
        .post(`/signup${userType}`, newUserData)
        .then((res) => {
            setAuthorizationHeader(res.data.token);
        });
};
...
export function googleSignIn(token){
    return axios
        .post('/getToken',token)
        .then((res) => {
            setAuthorizationHeader(res.data.stsTokenManager.accessToken)
        });
}
